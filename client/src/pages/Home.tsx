...
function QrIdentity({ isArabic, tx }: { isArabic: boolean; tx: (en: string, ar: string) => string }) {
  ...
  return (
    <section className="section qr-section scroll-section" id="qr" aria-labelledby="qr-title">
      ...
      <div className="qr-card" id="besma-qr"><div className="qr-frame"><QRCodeCanvas value={url} size={250} level="H" bgColor="#f2f0ea" fgColor="#0a0a09" includeMargin /><QRCodeSVG value={url} size={250} level="H" bgColor="#f2f0ea" fgColor="#0a0a09" includeMargin className="qr-svg-source" /><img src="/assets/besma-portrait.png" alt={tx("Besma Kaddour profile image", "صورة ملف بسمة قدور")} /><span className="qr-corner qr-corner-tl" /><span className="qr-corner qr-corner-br" /></div><div className="qr-url">{tx("SCAN / BESMA KADDOUR", "امسحي / بسمة قدور")}<span>{url.replace(/^https?:\/\//, "")}</span></div><div className="qr-actions"><button type="button" className="button button-quiet" onClick={() => downloadQr("png")}>{tx("Download PNG", "تحميل PNG")} <Download size={14} /></button><button type="button" className="button button-quiet" onClick={() => downloadQr("svg")}>{tx("Download SVG", "تحميل SVG")} <Download size={14} /></button></div></div>
    </section>
  );
}
