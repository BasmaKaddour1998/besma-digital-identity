  ...
  return (
    <div className={`site-shell ${isArabic ? "is-arabic" : ""} ${isFrench ? "is-french" : ""} ${isLight ? "theme-light" : ""}`}>
      ...
      <main id="main-content">
        ...
        <section className="section section-cv section-cv-modern scroll-section" id="cv" aria-labelledby="cv-title">
          ...
          <div className="cv-modern-banner"><span className="cv-banner-mark">BK / 01</span><span>{tx("ENGINEERING · AI · PRODUCT", "هندسة · ذكاء اصطناعي · منتجات")}</span><span className="cv-banner-line" /></div><div className="cv-grid"><img className="section-watermark cv-watermark" src="/assets/besma-signature.png" alt="" aria-hidden="true" /><div><div className="cv-card-kicker">{tx("A concise professional record", "سجل مهني مختصر")}</div><p className="cv-profile">{tx("Professional profile and experience — choose a language to download.", "الملف المهني والخبرة — اختاري اللغة للتحميل.")}</p><div className="cv-actions"><a className="button button-primary" href="/cv-besma-en.pdf" download="Besma-Kaddour-CV-English.pdf">{tx("CV · EN", "السيرة · EN")} <Download size={15} /></a><a className="button button-quiet" href="/cv-besma-ar.pdf" download="Besma-Kaddour-CV-Arabic.pdf">{tx("CV · AR", "السيرة · AR")} <Download size={15} /></a><a className="button button-quiet" href="/cv-besma-fr.pdf" download="Besma-Kaddour-CV-French.pdf">{tx("CV · FR", "السيرة · FR", "CV · FR")} <Download size={15} /></a></div></div><div className="cv-index"><div className="cv-index-heading">{tx("Inside this CV", "محتويات السيرة")}</div>{["Profile", "Experience", "Education", "Skills", "Technologies", "Projects", "Languages", "Achievements"].map((item, index) => <span key={item}><b>0{index + 1}</b>{isArabic ? ["الملف الشخصي", "الخبرة", "التعليم", "المهارات", "التقنيات", "المشاريع", "اللغات", "الإنجازات"][index] : item}</span>)}</div></div>
          <article className="cv-certificate-feature"><div className="cv-certificate-copy"><span className="eyebrow">{tx("Certification", "شهادة")}</span><h3>{tx("NASA International Space Apps Challenge", "NASA International Space Apps Challenge")}</h3><p>{tx("GALACTIC PROBLEM SOLVER · October 5–6, 2024", "GALACTIC PROBLEM SOLVER · 5–6 أكتوبر 2024")}</p></div><img src="/assets/nasa-certificate.png" alt={tx("NASA Galactic Problem Solver certificate awarded to Besma Kaddour", "شهادة NASA Galactic Problem Solver المقدمة إلى بسمة قدور")} loading="lazy" /></article>
        </section>
